...
export async function getCryptoData() {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/crypto`, {
      cache: "no-store",
    });
...
export async function getProductivityData() {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/productivity`, {
      cache: "no-store",
    });
