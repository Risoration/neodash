...
interface SetupPayload {
  crypto?: {
    ...
    apiKey?: string;
    apiSecret?: string;
    wallets?: Array<{ chain: "ethereum" | "bitcoin" | "solana"; address: string }>;
  };
  weather?: {
    location: string;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body.weather) {
      ...
      const temperatureUnit = userConfig?.preferences?.temperatureUnit || "fahrenheit";
    ...
    if (body.productivity) {
      updates.push(saveProductivityGoals(user.id, body.productivity.goals));
    }
...
async function saveCryptoData(userId: string, cryptoData: SetupPayload["crypto"]) {
  if (!cryptoData) {
  ...
  } else if (method === "exchange" && cryptoData.apiKey && cryptoData.apiSecret) {
  ...
  const change24h =
    coins.length > 0
      ? coins.reduce((acc, coin) => acc + (coin.change24h || 0), 0) / coins.length
      : 0;
...
async function saveFinancialData(
  ...
  const accounts: FinancialAccount[] = financialData.accounts.map((acc) => ({
    ...
    lastSynced: acc.linkedVia === "plaid" ? new Date().toISOString() : undefined,
  }));
...
async function saveProductivityGoals(
  userId: string,
  goals?: SetupPayload["productivity"]["goals"]
) {
  if (!goals) {
